...
<body>
	<div style="width: 100%; background-color: #150a50">
		<table align="center" width="60%" style="background-color: #150a50" border="0">
			<tr>
				<td>
					<a href="consulta_geo.php"><button class="sele btn btn-success">Predio? clique aqui</button></a>
				</td>
				<td align="center">
					<a href="index.php">
				...
				<td  style="width: 230px">
					<font style="font-weight: bold; font-size: 24px; font-family: Verdana; color: white"> Pesquisar Casa </font>
				</td>
			</tr>
			<tr>
				<td></td>
				<td align="left" style='color: white'>
					Você está em: <b><a href="index.php" class="wher">Inicio-></a>Casa</b>
				</td>
			</tr>
		</table>
	</div>
		...
		<?php
			if(isset($_GET['submit'])){
				...
				if(($cep =="") and ($logradouro == "")){
					...
					$query = "SELECT * FROM casa where cep like '%".$cep."%' and logradouro like '%".$logradouro."%'  LIMIT 100 ";
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">';
			...
			while($row1 = $result1->fetch_assoc()){
				...
				if( ($row1['tipo4']== '=') and ($ruaaa == 0) and ('predio' != $row1['param0']) ){
					if($row['logradouro'] == $row1['param4']){
						$promocao = $row1['promocao'];
						$ruaaa = 1;
					}
				}else{
					if( ($row1['tipo3']== '=') and ($bairroo==0)  and ($ruaaa == 0) and ('predio' != $row1['param0']) ){
						if($row['bairro'] == $row1['param3']){
							$promocao = $row1['promocao'];
							$bairroo = 1;
						}
					}else{
						if( ($row1['tipo2']== '=') and ($cidadee==0) and ($bairroo==0)  and ($ruaaa == 0) and ('predio' != $row1['param0']) ){
							if(($row['cidade'] == $row1['param2']) and ($row1['param1']==$tec)){
								$promocao = $row1['promocao'];
								$cidadee = 1;
							}
						}else{
							if( ($row1['tipo1']== '=') and ($tecnologiaa==0)  and ($cidadee==0) and ($bairroo==0)  and ($ruaaa == 0) and ('predio' != $row1['param0']) ){
								if($row['tecnologia'] == $row1['param1']){
									$promocao = $row1['promocao'];
									$tecnologiaa = 1;
								}
							}else{
								if( ($row1['tipo0']== '=') and ($tipoo==0)  and ($tecnologiaa==0)  and ($cidadee==0) and ($bairroo==0)  and ($ruaaa == 0) and ('predio' != $row1['param0']) ){
									if('predio' != $row1['param0']){
										$promocao = $row1['promocao'];
										$tipoo=1;
									}
								}else if( ($tipoo ==0)  and ($tecnologiaa==0)  and ($cidadee==0) and ($bairroo==0)  and ($ruaaa == 0) and ('predio' != $row1['param0']) ){
									$promocao = $row1['promocao'];
								}
